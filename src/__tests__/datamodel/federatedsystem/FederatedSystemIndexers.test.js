...
import { FederatedSystemIndexers } from '../../../datamodel/federatedsystem';
import {
  mockFetch,
  mockWaitingFetch,
  restoreFetch,
...
function testInvalidData(data) {
  ...
  const { container } = render(<FederatedSystemIndexers system={data} />);
...
test(`FederatedSystemIndexers receives null data `, async () => {
  testInvalidData(null);
});

test(`FederatedSystemIndexers receives data with NO 'indexers' `, async () => {
  const data = {};
  testInvalidData(data);
});

test(`FederatedSystemIndexers receives data with empty 'indexers' `, async () => {
  const data = { indexers: [] };
  testInvalidData(data);
});

test(`FederatedSystemIndexers structure and behavior with valid data' `, async () => {
  let topContainer = null;

  // create a FederatedSystemIndexers:
  ...
  await act(async () => {
    const { container } = render(
      <RuntimedataContextProvider>
        <FederatedSystemIndexers system={systemJson} />
      </RuntimedataContextProvider>
    );
  ...
  expect(screen.getByRole('heading', { name: 'Indexers' })).toBeDefined();
